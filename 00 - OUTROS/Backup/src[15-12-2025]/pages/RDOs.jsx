...
const formatDateForInput = (dateString) => {
    ...
    const carregarDados = async () => {
      try {
        ...
        const [projetosRes, rdosRes] = await Promise.all([
          ...
          fetch(`${API_URL}/api/rdos${usuario && !usuario.admin ? `?usuario_id=${usuario.id}` : ''}`)
        ]);
  ...
  if (loading || !usuario || projetos.length === 0) {
    return (
      <div className={`${isMobile ? 'min-h-dvh p-4 flex items-center justify-center' : 'min-h-screen p-8 flex items-center justify-center'}`}>
  ...
  const rdosFiltrados = rdos.filter((rdo) => {
    ...
    if (isAdmin && filtros.funcionarioId && String(rdo.usuario_id) !== filtros.funcionarioId) return false;
    return true;
  });
  ...
  const funcionariosUnicos = isAdmin ?
    [...new Map(rdos.map(r => [r.usuario_id, { id: r.usuario_id, nome: r.usuario_nome }])).values()]
  ...
  const calcularResumoRDO = (horarios, horasNormaisPorDia) => {
    let totalHoras = 0, horasNoturnas = 0, horasDeslocamentoTotal = 0;
  ...
  const resumoForm = projetoForm ? calcularResumoRDO(formData.horarios, horasNormaisForm) : { totalHoras: 0, horasExtras: 0, horasNoturnas: 0, horasNormais: 0 };


  const addHorarioNormal = () => {
    if (!window.confirm("Este comando irá apagar TODOS os horários deste RDO e substituir por 08:00–12:00 e 13:00–17:00. Deseja continuar?")) return;
    setFormData((prev) => ({
      ...prev,
      horarios: [
  ...
  return (
    <>
      ...
      `}>
        ...
        {/* CONTADOR */}
        <div className={`bg-gradient-to-r from-emerald-500 to-teal-600 text-white p-${isMobile ? '4 sm:p-6' : '6'} rounded-2xl shadow-xl mb-6 sm:mb-8`}>
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div>
              <h2 className={`text-${isMobile ? 'xl sm:text-2xl' : '2xl'} font-bold`}>Total do Período</h2>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className={`px-${isMobile ? '4 sm:px-6' : '6'} py-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white`}>
            <div className="flex items-center justify-between">
              <h2 className={`text-${isMobile ? 'lg sm:text-xl' : 'xl'} font-bold`}>
                {isAdmin ? "RDOs (Admin)" : "Meus RDOs"}
              </h2>
              <span className={`text-${isMobile ? 'xs sm:text-sm' : 'sm'} bg-blue-400 px-3 py-1 rounded-full`}>
        ...
        <div key={rdo.id} className="p-4 hover:bg-gray-50 transition-colors">
          ...
          <div className="flex justify-between items-start mb-3">
            <div>
              ...
              {isAdmin && (
                <div className="text-sm text-gray-600 mt-1">{rdo.usuario_nome || "—"}</div>
              )}
            </div>
            <div className="text-right">
              <div className="text-xl font-black text-emerald-600">{resumo.totalHoras.toFixed(1)}h</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-4 text-center p-2 bg-gray-50 rounded-lg">
            <div>
              <div className="font-bold text-orange-600 text-sm">{resumo.horasExtras.toFixed(1)}h</div>
              <div className="text-xs text-gray-500">HE</div>
            </div>
            <div>
              <div className="font-bold text-indigo-600 text-sm">{resumo.horasNoturnas.toFixed(1)}h</div>
            ...
            <button
              onClick={() => {
                setFormData({
                  ...
                  horarios: rdo.horarios || [{ hora_inicio: "", hora_fim: "", titulo: "", atividade: "Deslocamento" }],
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    {isAdmin && <th className="px-6 py-4 text-left text-xs font-bold uppercase text-gray-600">Funcionário</th>}
                ...
                <tbody className="divide-y divide-gray-200">
                  {rdosOrdenados.map((rdo) => {
                    ...
                    return (
                      <tr key={rdo.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        {isAdmin && (
                          <td className="px-6 py-3 text-sm text-gray-800">{rdo.usuario_nome || "—"}</td>
                        )}
                        ...
                        <td className="px-4 py-3">
                          <div className="text-lg font-bold text-emerald-600">{resumo.totalHoras.toFixed(1)}h</div>
                  ...
                  {rdosOrdenados.length === 0 && (
                    <tr>
                      <td colSpan={isAdmin ? 8 : 7} className="px-6 py-6 text-center text-sm text-gray-500">
                        Nenhum RDO encontrado com os filtros atuais.
                      </td>
                    </tr>
                  )}
      ...
      {showForm && (
        ...
        `}>
          ...
          `}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 px-0 sm:px-0 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">🏗️ Projeto *</label>
                  <select
                    name="projeto_id"
              ...
              <div>
                ...
                <div className="space-y-4 max-h-80 lg:max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-blue-300 scrollbar-track-blue-100 rounded-lg p-4 bg-gray-50">
                  {formData.horarios.map((horario, index) => (
                    <div key={index} className="border border-gray-200 p-6 rounded-2xl bg-white shadow-sm">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-sm font-medium mb-2">Código da Atividade</label>
                          <select
